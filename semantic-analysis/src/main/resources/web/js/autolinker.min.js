...
var Autolinker = {
	...
	link : function(h, d) {
		d = d || {};
		var k = Autolinker.htmlRegex, r = Autolinker.matcherRegex, i = ("newWindow" in d) ? d.newWindow
				: true, e = ("stripPrefix" in d) ? d.stripPrefix : true, a = d.truncate, g = ("twitter" in d) ? d.twitter
				: true, o = ("email" in d) ? d.email : true, s = ("urls" in d) ? d.urls
				: true, j, q = 0, b, m = "", l = 0;
		function f(t) {
			t = t
					.replace(
							r,
							function(O, A, x, w, v, u) {
								var E = A, D = x, L = w, y = v, F = u, C = "", B = "", M = [];
								var G = O.charAt(O.length - 1);
								if (G === ")") {
									var z = O.match(/\(/g), J = O.match(/\)/g), N = (z && z.length) || 0, K = (J && J.length) || 0;
									if (N < K) {
								...
								if (E) {
									...
									if (y) {
										...
										if (!/^[A-Za-z]{3,9}:/i.test(I)) {
											I = "http://" + I
										}
									}
								}
								if (e) {
									H = H.replace(Autolinker.prefixRegex, "")
								}
		...
		while ((j = k.exec(h)) !== null) {
			var n = j[0], c = j[2], p = !!j[1];
			b = h.substring(q, j.index);
			q = j.index + n.length;
			if (c === "a") {
				if (!p) {
					l++;
